...
MultivariatePolynomialKokkos::MultivariatePolynomialKokkos(
    int num_variables, std::vector<double> coefficients, std::vector<std::vector<int>> monomials) {
    ...
    for (auto m : monomials)
        if (m.size() > max_monomial_size) max_monomial_size = m.size();
    ...
    for (int i = 0; i < mp.nodes.size(); ++i)
        for (int j = 0; j < mp.nodes[i].size(); ++j) h_nodes(i, j) = mp.nodes[i][j];
    ...
    for (int i = 0; i < mp.edges.size(); ++i)
        for (int j = 0; j < 2; ++j) h_edges(i, j) = mp.edges[i][j];
...
double MultivariatePolynomialKokkos::evaluate(const Kokkos::View<const double*>& x) {
    initialize_forward_pass(x);
    forward_pass();
    return KokkosBlas::dot(node_coefficients, node_values);
}

double MultivariatePolynomialKokkos::evaluate_simple(const Kokkos::View<const double*>& x) {
    double f;
    Kokkos::parallel_reduce(
        "MultivariatePolynomialKokkos::evaluate_simple", coefficients.size(),
...
double MultivariatePolynomialKokkos::evaluate_gradient(const Kokkos::View<const double*>& x,
...
double MultivariatePolynomialKokkos::evaluate_gradient_simple(const Kokkos::View<const double*>& x,
                                                              Kokkos::View<double*>& g) {
    double f;
    Kokkos::parallel_reduce(
        "MultivariatePolynomialKokkos::evaluate_gradient_simple", coefficients.size(),
        KOKKOS_CLASS_LAMBDA(const int i, double& local_result) {
            double monomial = x(monomials(i, 0));
            for (int j = 1; j < monomials.extent(1); ++j) {
                if (monomials(i, j) == -1) break;
...
void MultivariatePolynomialKokkos::initialize_forward_pass(const Kokkos::View<const double*>& x) {
    Kokkos::parallel_for(
        "MultivariatePolynomialKokkos::initialize_forward_pass", num_variables,
        KOKKOS_CLASS_LAMBDA(const int i) { node_values(i) = x(i); });
}
...
void MultivariatePolynomialKokkos::initialize_backward_pass() {
    Kokkos::parallel_for(
        "MultivariatePolynomialKokkos::initialize_backward_pass", node_coefficients.size(),
        KOKKOS_CLASS_LAMBDA(const int i) { node_adjoints(i) = node_coefficients(i); });
}
...
void MultivariatePolynomialKokkos::extract_gradient_from_graph(Kokkos::View<double*>& g) {
    Kokkos::deep_copy(g, 0.0);
    Kokkos::parallel_for(
        "MultivariatePolynomialKokkos::extract_gradient_from_graph", num_variables,
        KOKKOS_CLASS_LAMBDA(const int i) { g(i) = node_adjoints(i); });
}
