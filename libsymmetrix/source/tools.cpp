...
int _sum(std::vector<int> v) { return std::reduce(v.begin(), v.end()); }

std::vector<std::vector<std::vector<int>>> _partitions(std::vector<int> v) {
    if (v.size() == 0) return std::vector<std::vector<std::vector<int>>>();
...
std::vector<std::vector<std::vector<int>>> _two_part_partitions(std::vector<int> v) {
    ...
    std::sort(partitions.begin(), partitions.end(),
              [](const std::vector<std::vector<int>>& i, const std::vector<std::vector<int>>& j) {
                  return i.size() < j.size();
              });
    return partitions;
}
...
std::vector<std::vector<int>> _permutations(int size) {
    ...
    while (std::next_permutation(v.begin(), v.end())) permutations.push_back(v);
    return permutations;
}

std::vector<std::vector<int>> _combinations(std::vector<int> set, int k) {
    if (k != 2) throw std::invalid_argument("_combinations: only k=2 is implemented.");
    std::vector<std::vector<int>> combinations;
    for (int i = 0; i < set.size(); ++i)
        for (int j = i + 1; j < set.size(); ++j) combinations.push_back({set[i], set[j]});
    return combinations;
}

std::vector<std::vector<int>> _product_repeat(std::vector<int> input, int repeat) {
    auto inputs = std::vector<std::vector<int>>();
    for (int i = 0; i < repeat; ++i) inputs.push_back(input);
...
std::vector<std::vector<int>> _generate_indices(int dim, int max) {
    ...
    for (int r = 0; r < dim; ++r) inputs_to_prod.push_back(indices);
