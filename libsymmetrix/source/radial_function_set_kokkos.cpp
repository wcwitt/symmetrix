...
template <typename Precision>
RadialFunctionSetKokkos<Precision>::RadialFunctionSetKokkos(
    double h, std::vector<std::vector<std::vector<double>>> node_values,
    ...
    auto c = Kokkos::View<Precision****, Kokkos::LayoutRight>("coefficients", num_edge_types,
                                                              num_nodes - 1, 4, num_functions);
            ...
            for (int j = 0; j < num_functions; ++j) {
                ...
                h_c(a, i, 2, j) = static_cast<Precision>(
                    (-3 * node_values[a][j][i] - 2 * h * node_derivatives[a][j][i] +
                     3 * node_values[a][j][i + 1] - h * node_derivatives[a][j][i + 1]) /
                    (h * h));
                h_c(a, i, 3, j) = static_cast<Precision>(
                    (2 * node_values[a][j][i] + h * node_derivatives[a][j][i] -
                     2 * node_values[a][j][i + 1] + h * node_derivatives[a][j][i + 1]) /
                    (h * h * h));
            }
        }
    }
...
void RadialFunctionSetKokkos<Precision>::evaluate(
    ...
    Kokkos::View<const int*> neigh_types, Kokkos::View<const double*> r,
    Kokkos::View<Precision**, Kokkos::LayoutRight> R,
    Kokkos::View<Precision**, Kokkos::LayoutRight> R_deriv) const {
    const auto h = this->h;
    const auto num_functions = this->num_functions;
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            ...
            // compute function values
            Kokkos::parallel_for(
                Kokkos::TeamVectorRange(team_member, num_functions), [&](const int k) {
                    ...
                    R(ij, k) = c0 + c1 * static_cast<Precision>(x) +
                               c2 * static_cast<Precision>(xx) + c3 * static_cast<Precision>(xxx);
                    R_deriv(ij, k) = c1 + c2 * static_cast<Precision>(two_x) +
