...
MultilayerPerceptron::MultilayerPerceptron(std::vector<int> shape,
                                           std::vector<std::vector<double>> weights,
                                           double activation_scale_factor)
    : shape(shape), weights(weights), activation_scale_factor(activation_scale_factor) {
...
auto MultilayerPerceptron::evaluate(std::vector<double> input) -> std::vector<double> {
...
auto MultilayerPerceptron::evaluate_gradient(std::vector<double> input)
    ...
    for (int l = 0; l < shape.size() - 2; ++l) {
        ...
        for (int i = 0; i < shape[l + 1]; ++i) {
            ...
            node_activation_derivs[l + 1][i] =
                activation_scale_factor * sigmoid +
...
auto MultilayerPerceptron::evaluate_batch(std::vector<double> input, const int batch_size)
    -> std::vector<double> {
    // TODO: Check/sanitize input
    // Reshape node arrays and send input to nodes
    for (int i = 0; i < shape.size(); ++i) {
...
auto MultilayerPerceptron::evaluate_gradient_batch(std::vector<double> input, const int batch_size)
    -> std::tuple<std::vector<double>, std::vector<double>> {
    // TODO: Check/sanitize input
    // Reshape node arrays and send input to nodes
    for (int i = 0; i < shape.size(); ++i) {
    ...
    for (int i = 0; i < batch_size; ++i) {
        std::copy(weights.back().begin(), weights.back().end(),
                  node_derivs.end()[-2].begin() + i * weights.back().size());
    }
    for (int l = shape.size() - 3; l >= 0; --l) {
        for (int i = 0; i < batch_size; ++i) {
            for (int j = 0; j < shape.back(); ++j) {
                for (int p = 0; p < shape[l + 1]; ++p) {
                    node_derivs[l + 1][i * shape.back() * shape[l + 1] + j * shape[l + 1] + p] *=
                        node_activation_derivs[l + 1][i * shape[l + 1] + p];
                }
            }
        }
        cblas_dgemm(CblasRowMajor,              // const CBLAS_LAYOUT Layout
