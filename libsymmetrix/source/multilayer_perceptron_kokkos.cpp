...
MultilayerPerceptronKokkos::MultilayerPerceptronKokkos(std::vector<int> shape,
    ...
    for (int l = 0; l < shape.size() - 1; ++l) {
        ...
        for (int i = 0; i < shape[l + 1]; ++i)
            for (int j = 0; j < shape[l]; ++j) h_w(i, j) = weights[l][i * shape[l] + j];
    ...
    this->node_values =
        Kokkos::View<Kokkos::View<double**, Kokkos::LayoutRight>*, Kokkos::SharedSpace>(
            Kokkos::view_alloc("node_values", Kokkos::SequentialHostInit), shape.size());

    this->node_derivatives =
        Kokkos::View<Kokkos::View<double**, Kokkos::LayoutRight>*, Kokkos::SharedSpace>(
...
void MultilayerPerceptronKokkos::evaluate(Kokkos::View<const double**, Kokkos::LayoutRight> x,
    ...
    for (int l = 0; l < shape.size(); ++l) {
        ...
        if (node_derivatives(l).extent(0) != batch_size or
    ...
    for (int i = 0; i < batch_size; ++i) {
        for (int l = 0; l < shape.size() - 1; ++l) {
            ...
            KokkosBlas::SerialGemv<KokkosBatched::Trans::NoTranspose,
                                   KokkosBatched::Algo::Gemv::Unblocked>::invoke(1.0, weights(l),
                                                                                 in, 0.0, out);
            if (l == shape.size() - 2) break;  // no activation in final layer
            Kokkos::parallel_for(
                "MultilayerPerceptronKokkos::evaluate::activation", shape(l + 1),
...
void MultilayerPerceptronKokkos::evaluate_gradient(
    ...
    Kokkos::View<double*, Kokkos::LayoutRight> f, Kokkos::View<double**, Kokkos::LayoutRight> g) {
    const int batch_size = x.extent(0);
    for (int l = 0; l < shape.size(); ++l) {
        if (node_values(l).extent(0) != batch_size or node_values(l).extent(1) != shape(l))
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_CLASS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            ...
            for (int l = 0; l < shape.size() - 1; ++l) {
                Kokkos::parallel_for(
                    Kokkos::TeamThreadRange(team_member, weights(l).extent(0)), [=](const int p) {
                ...
                if (l == shape.size() - 2) break;  // no activation in final layer
                Kokkos::parallel_for(
                    Kokkos::TeamThreadRange(team_member, shape(l + 1)), [=](const int j) {
                        node_values(l + 1)(i, j) = activation_scale * node_values(l + 1)(i, j) /
                                                   (1.0 + Kokkos::exp(-node_values(l + 1)(i, j)));
                    });
                team_member.team_barrier();
            }
            // ----- reverse -----
            ...
            for (int l = shape.size() - 3; l >= 0; --l) {
                ...
                team_member.team_barrier();
                Kokkos::parallel_for(Kokkos::TeamThreadRange(team_member, shape(l + 1)),
                                     [=](const int j) {
                                         const double x = node_values(l + 1)(i, j);
                                         const double sigmoid = 1.0 / (1.0 + std::exp(-x));
                                         node_derivatives(l + 1)(i, j) *=
                                             activation_scale * sigmoid +
                                             activation_scale * x * sigmoid * (1 - sigmoid);
                                     });
                team_member.team_barrier();
                Kokkos::parallel_for(Kokkos::TeamThreadRange(team_member, weights(l).extent(1)),
                                     [=](const int p) {
                                         node_derivatives(l)(i, p) = 0.0;
                                         for (int q = 0; q < weights(l).extent(0); ++q)
                                             node_derivatives(l)(i, p) +=
                                                 weights(l)(q, p) * node_derivatives(l + 1)(i, q);
                                     });
                team_member.team_barrier();
            }
        });
