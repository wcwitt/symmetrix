...
ZBLKokkos::ZBLKokkos(double a_exp, double a_prefactor, std::vector<double> c,
                     std::vector<double> covalent_radii, int p)
    : a_exp(a_exp), a_prefactor(a_prefactor), p(p) {
...
void ZBLKokkos::compute_ZBL(const int num_nodes, Kokkos::View<const int*> node_types,
                            Kokkos::View<const int*> num_neigh,
                            Kokkos::View<const int*> neigh_types,
                            Kokkos::View<const int*> atomic_numbers, Kokkos::View<const double*> r,
                            Kokkos::View<const double*> xyz, Kokkos::View<double*> node_energies,
    ...
    Kokkos::parallel_for(
        "ZBLKokkos::compute_ZBL", Kokkos::TeamPolicy<>(num_nodes, Kokkos::AUTO),
        KOKKOS_CLASS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            ...
            Kokkos::parallel_reduce(
                ...
                e_i);
            Kokkos::single(Kokkos::PerTeam(team_member), [&]() { node_energies(i) += e_i; });
        });
}
