...
template <typename T>
Kokkos::View<T**> toKokkosView(std::string name, const std::vector<T>& vector, const int N0,
                               const int N1) {
    // TODO: sanitize input
    auto view = Kokkos::View<T**>(name, N0, N1);
    auto host_view = Kokkos::create_mirror_view(view);
    for (int i = 0; i < N0; ++i)
        for (int j = 0; j < N1; ++j) host_view(i, j) = vector[i * N1 + j];
...
Kokkos::View<T***> toKokkosView(std::string name, const std::vector<T>& vector, const int N0,
    ...
    for (int i = 0; i < N0; ++i)
        for (int j = 0; j < N1; ++j)
            for (int k = 0; k < N2; ++k) host_view(i, j, k) = vector[(i * N1 + j) * N2 + k];
    Kokkos::deep_copy(view, host_view);
    return view;
}
...
std::vector<T> view2vector(Kokkos::View<T*> view) {
    ...
    for (int i = 0; i < h_view.size(); ++i) vector[i] = h_view(i);
    return vector;
}
...
std::vector<T> view2vector(Kokkos::View<T**, Kokkos::LayoutRight> view) {
    ...
    for (int i = 0; i < h_view.extent(0); ++i)
        for (int j = 0; j < h_view.extent(1); ++j) vector[i * h_view.extent(1) + j] = h_view(i, j);
    return vector;
}
...
template <typename T>
void set_kokkos_view(Kokkos::View<T*>& view, std::vector<T> array) {
    auto d_array = Kokkos::create_mirror_view_and_copy(
        Kokkos::DefaultExecutionSpace(),
        Kokkos::View<const T*, Kokkos::HostSpace, Kokkos::MemoryUnmanaged>(array.data(),
                                                                           array.size()));
    if (view.size() != array.size()) Kokkos::realloc(view, array.size());
    Kokkos::deep_copy(view, d_array);
}

template <typename T>
void set_kokkos_view(Kokkos::View<T**, Kokkos::LayoutRight>& view, std::vector<T> array,
                     const int N0, const int N1) {
    auto d_array = Kokkos::create_mirror_view_and_copy(
        Kokkos::DefaultExecutionSpace(),
        Kokkos::View<const T**, Kokkos::LayoutRight, Kokkos::HostSpace, Kokkos::MemoryUnmanaged>(
            array.data(), N0, N1));
    if (view.extent(0) != N0 or view.extent(1) != N1) Kokkos::realloc(view, N0, N1);
    Kokkos::deep_copy(view, d_array);
}
