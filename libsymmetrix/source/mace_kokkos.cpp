...
void MACEKokkos<Precision>::compute_node_energies_forces(
    const int num_nodes, Kokkos::View<const int*> node_types, Kokkos::View<const int*> num_neigh,
    Kokkos::View<const int*> neigh_indices, Kokkos::View<const int*> neigh_types,
    Kokkos::View<const double*> xyz, Kokkos::View<const double*> r) {
    if (node_energies.size() < num_nodes) Kokkos::realloc(node_energies, num_nodes);
    if (node_forces.size() < xyz.size()) Kokkos::realloc(node_forces, xyz.size());
    Kokkos::deep_copy(node_energies, 0.0);
    Kokkos::deep_copy(node_forces, 0.0);

    if (has_zbl)
        zbl.compute_ZBL(num_nodes, node_types, num_neigh, neigh_types, atomic_numbers, r, xyz,
...
template <typename Precision>
void MACEKokkos<Precision>::compute_R0(const int num_nodes, Kokkos::View<const int*> node_types,
    ...
    Kokkos::parallel_scan(
        "first_neigh", num_nodes, KOKKOS_LAMBDA(const int i, int& update, const bool final) {
            const int num_neigh_i = num_neigh(i);
            if (final) first_neigh(i) = update;
            update += num_neigh_i;
        });
    Kokkos::fence();
    Kokkos::View<int*> i_list("i_list", r.size());
    Kokkos::parallel_for(
        "ij lists", num_nodes, KOKKOS_LAMBDA(const int i) {
    ...
    Kokkos::parallel_for(
        "Compute R0", Kokkos::TeamPolicy<>(r.size(), Kokkos::AUTO, 32),
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            ...
            Kokkos::parallel_for(Kokkos::TeamVectorRange(team_member, (l_max + 1) * num_channels),
                                 [&](const int lk) {
...
template <typename Precision>
void MACEKokkos<Precision>::compute_R1(const int num_nodes, Kokkos::View<const int*> node_types,
                                       Kokkos::View<const int*> num_neigh,
                                       Kokkos::View<const int*> neigh_types,
                                       Kokkos::View<const double*> r) {
...
void MACEKokkos<Precision>::compute_Y(Kokkos::View<const double*> xyz) {
    ...
    if (xyz_shuffled.extent(0) < 3 * num) Kokkos::realloc(xyz_shuffled, 3 * num);
    auto xyz_shuffled = this->xyz_shuffled;
    Kokkos::parallel_for(
        "shuffle_xyz", num, KOKKOS_LAMBDA(int i) {
    ...
    sphericart.compute_array_with_gradients(h_xyz.data(), 3 * num, h_Y.data(), 3 * num * num_lm,
                                            h_Y_grad.data(), 3 * num * num_lm),
    ...
    Kokkos::parallel_for(
        "unshuffle_Y_grad", num, KOKKOS_LAMBDA(int i) {
            for (int lm = 0; lm < num_lm; ++lm) {
                Y_grad(3 * i * num_lm + 0 * num_lm + lm) =
                    Y_grad_shuffled(3 * i * num_lm + 1 * num_lm + lm);
                Y_grad(3 * i * num_lm + 1 * num_lm + lm) =
                    Y_grad_shuffled(3 * i * num_lm + 2 * num_lm + lm);
                Y_grad(3 * i * num_lm + 2 * num_lm + lm) =
                    Y_grad_shuffled(3 * i * num_lm + 0 * num_lm + lm);
            }
        });
    Kokkos::fence();
    ...
    Kokkos::parallel_for(
        "normalize_Y", num * num_lm, KOKKOS_LAMBDA(int i) { Y(i) *= 2 * std::sqrt(M_PI); });
    Kokkos::parallel_for(
        "normalize_Y_grad", 3 * num * num_lm,
    ...
    Kokkos::parallel_for(
        "normalize_Y", num * num_lm, KOKKOS_LAMBDA(int i) { Y(i) *= normalization_factor; });
    Kokkos::parallel_for(
        "normalize_Y_grad", 3 * num * num_lm,
        KOKKOS_LAMBDA(int i) { Y_grad(i) *= normalization_factor; });
...
template <typename Precision>
void MACEKokkos<Precision>::compute_A0(const int num_nodes, View<const int*> node_types,
                                       View<const int*> num_neigh, View<const int*> neigh_types) {
    if (A0.extent(0) != num_nodes) Kokkos::realloc(A0, num_nodes, num_lm, num_channels);
    Kokkos::deep_copy(A0, 0.0);

    Kokkos::View<int*> first_neigh("first_neigh", num_nodes);
    Kokkos::parallel_scan(
        "Compute first_neigh", num_nodes,
    ...
    parallel_for(
        ...
        KOKKOS_LAMBDA(TeamPolicy<>::member_type team_member) {
            ...
            for (int j = 0; j < num_neigh(i); ++j) {
                ...
                parallel_for(TeamVectorRange(team_member, num_channels), [=](const int k) {
...
template <typename Precision>
void MACEKokkos<Precision>::reverse_A0(const int num_nodes, Kokkos::View<const int*> node_types,
                                       Kokkos::View<const int*> num_neigh,
                                       Kokkos::View<const int*> neigh_types,
    ...
    Kokkos::parallel_for(
        "Reverse A0", Kokkos::TeamPolicy<>(num_nodes, Kokkos::AUTO, 32),
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
...
template <typename Precision>
void MACEKokkos<Precision>::compute_A0_scaled(const int num_nodes,
    ...
    A0_splines.evaluate(num_nodes, node_types, num_neigh, neigh_types, r, A0_spline_values,
                        A0_spline_derivs);
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            ...
            Kokkos::parallel_reduce(
                ...
                [=](const int j, double& lsum) { lsum += A0_spline_values(first_neigh(i) + j, 0); },
                A0_scale_factor);
            A0_scale_factor += 1.0;
            ...
            Kokkos::parallel_for(
                ...
                [=](int lmk) { A0(i, lmk / num_channels, lmk % num_channels) /= A0_scale_factor; });
        });
    Kokkos::fence();
}

template <typename Precision>
void MACEKokkos<Precision>::reverse_A0_scaled(const int num_nodes,
                                              Kokkos::View<const int*> node_types,
                                              Kokkos::View<const int*> num_neigh,
                                              Kokkos::View<const int*> neigh_types,
                                              Kokkos::View<const double*> xyz,
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            ...
            Kokkos::parallel_reduce(
                ...
                dA0_dot_A0);
            team_member.team_barrier();
            Kokkos::parallel_for(Kokkos::TeamThreadRange(team_member, num_neigh(i)),
                                 [=](const int j) {
                                     const int ij = first_neigh(i) + j;
                                     const double f = A0_spline_values(ij, 0);
                                     const double d = A0_spline_derivs(ij, 0);
                                     node_forces(3 * ij + 0) +=
                                         dA0_dot_A0 / A0_scale_factor * d * xyz(3 * ij + 0) / r(ij);
                                     node_forces(3 * ij + 1) +=
                                         dA0_dot_A0 / A0_scale_factor * d * xyz(3 * ij + 1) / r(ij);
                                     node_forces(3 * ij + 2) +=
                                         dA0_dot_A0 / A0_scale_factor * d * xyz(3 * ij + 2) / r(ij);
                                 });
            // update dE/dA0
            Kokkos::parallel_for(
                Kokkos::TeamThreadRange(team_member, num_lm * num_channels), [=](int lmk) {
...
void MACEKokkos<Precision>::compute_M0(int num_nodes, Kokkos::View<const int*> node_types) {
    if (M0.extent(0) < num_nodes) Kokkos::realloc(M0, num_nodes, num_LM, num_channels);
    Kokkos::deep_copy(M0, 0.0);
    for (int LM = 0; LM < num_LM; ++LM) {
        if (M0_poly_values(LM).extent(0) < num_nodes)
            M0_poly_values(LM) = Kokkos::View<Precision***, Kokkos::LayoutRight>(
                Kokkos::view_alloc(std::string("M0_poly_values_") + std::to_string(LM),
    ...
    Kokkos::parallel_for(
        "Compute M0", Kokkos::TeamPolicy<>(num_nodes * num_LM, Kokkos::AUTO, 32),
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            const int i = team_member.league_rank() / num_LM;
            const int LM = team_member.league_rank() % num_LM;
            // initialize
            Kokkos::parallel_for(Kokkos::TeamVectorMDRange<Kokkos::Rank<2, Kokkos::Iterate::Right>,
                                                           Kokkos::TeamPolicy<>::member_type>(
                                     team_member, num_lm, num_channels),
                                 [&](const int lm, const int k) {
                                     M0_poly_values(LM)(i, lm, k) = A0(i, lm, k);
                                     Kokkos::atomic_add(&M0(i, LM, k),
                                                        M0_poly_coeff(LM)(node_types(i), lm, k) *
                                                            M0_poly_values(LM)(i, lm, k));
                                 });
            team_member.team_barrier();
            // forward pass
            for (int p = 0; p < M0_poly_spec(LM).extent(0); ++p) {
                ...
                Kokkos::parallel_for(
                    Kokkos::TeamVectorRange(team_member, num_channels), [&](const int k) {
                        M0_poly_values(LM)(i, num_lm + p, k) =
                            M0_poly_values(LM)(i, p0, k) * M0_poly_values(LM)(i, p1, k);
                        M0(i, LM, k) += M0_poly_coeff(LM)(node_types(i), num_lm + p, k) *
                                        M0_poly_values(LM)(i, num_lm + p, k);
                    });
            }
        });
...
void MACEKokkos<Precision>::reverse_M0(int num_nodes, Kokkos::View<const int*> node_types) {
    ...
    for (int LM = 0; LM < num_LM; ++LM) {
        if (M0_poly_adjoints(LM).extent(0) < num_nodes)
            M0_poly_adjoints(LM) = Kokkos::View<Precision***, Kokkos::LayoutRight>(
                Kokkos::view_alloc(std::string("M0_poly_adjoints_") + std::to_string(LM),
                                   Kokkos::WithoutInitializing),
                num_nodes, M0_poly_coeff(LM).extent(1), num_channels);
    }
    ...
    Kokkos::parallel_for(
        "Reverse M0", Kokkos::TeamPolicy<>(num_nodes * num_LM, Kokkos::AUTO, 32),
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            const int i = team_member.league_rank() / num_LM;
            const int LM = team_member.league_rank() % num_LM;
            // initialize
            Kokkos::parallel_for(Kokkos::TeamVectorMDRange<Kokkos::Rank<2, Kokkos::Iterate::Right>,
                                 ...
                                 [&](const int p, const int k) {
                                     M0_poly_adjoints(LM)(i, p, k) =
                                         M0_poly_coeff(LM)(node_types(i), p, k);
                                 });
            team_member.team_barrier();
            // backwards pass
            for (int p = M0_poly_spec(LM).extent(0) - 1; p >= 0; --p) {
                ...
                Kokkos::parallel_for(
                    Kokkos::TeamVectorRange(team_member, num_channels), [&](const int k) {
                        ...
                        M0_poly_adjoints(LM)(i, p0, k) +=
                            M0_poly_adjoints(LM)(i, num_lm + p, k) * M0_poly_values(LM)(i, p1, k);
                        M0_poly_adjoints(LM)(i, p1, k) +=
                            M0_poly_adjoints(LM)(i, num_lm + p, k) * M0_poly_values(LM)(i, p0, k);
                    });
            }
            team_member.team_barrier();
            Kokkos::parallel_for(Kokkos::TeamVectorMDRange<Kokkos::Rank<2, Kokkos::Iterate::Right>,
                                 ...
                                 [&](const int lm, const int k) {
                                     Kokkos::atomic_add(
                                         &A0_adj(i, lm, k),
                                         M0_poly_adjoints(LM)(i, lm, k) * M0_adj(i, LM, k));
                                 });
        });
    Kokkos::fence();
}
...
template <typename Precision>
void MACEKokkos<Precision>::compute_H1(const int num_nodes) {
    if (H1.extent(0) < M0.extent(0)) Kokkos::realloc(H1, M0.extent(0), M0.extent(1), M0.extent(2));
    ...
    Kokkos::parallel_for(
        "Compute H1", Kokkos::TeamPolicy<>(num_nodes * (L_max + 1), Kokkos::AUTO, Kokkos::AUTO),
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            const int i = team_member.league_rank() / (L_max + 1);
            const int l = team_member.league_rank() % (L_max + 1);
            auto M0_il =
                Kokkos::subview(M0, i, Kokkos::make_pair(l * l, l * (l + 2) + 1), Kokkos::ALL);
            auto W_il = Kokkos::subview(H1_weights, l, Kokkos::ALL, Kokkos::ALL);
            auto H1_il =
                Kokkos::subview(H1, i, Kokkos::make_pair(l * l, l * (l + 2) + 1), Kokkos::ALL);
...
template <typename Precision>
void MACEKokkos<Precision>::reverse_H1(const int num_nodes) {
    ...
    Kokkos::parallel_for(
        "Reverse H1", Kokkos::TeamPolicy<>(num_nodes * (L_max + 1), Kokkos::AUTO, Kokkos::AUTO),
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            const int i = team_member.league_rank() / (L_max + 1);
            const int l = team_member.league_rank() % (L_max + 1);
            auto H1_adj_il =
                Kokkos::subview(H1_adj, i, Kokkos::make_pair(l * l, l * (l + 2) + 1), Kokkos::ALL);
            auto W_il = Kokkos::subview(H1_weights, l, Kokkos::ALL, Kokkos::ALL);
            auto M0_adj_il =
                Kokkos::subview(M0_adj, i, Kokkos::make_pair(l * l, l * (l + 2) + 1), Kokkos::ALL);
            KokkosBatched::TeamGemm<Kokkos::TeamPolicy<>::member_type,
                                    KokkosBatched::Trans::NoTranspose,
...
template <typename Precision>
void MACEKokkos<Precision>::compute_Phi1(const int num_nodes, Kokkos::View<const int*> num_neigh,
                                         Kokkos::View<const int*> neigh_indices) {
    // Compute Phi1_lelm1lm2 (named Phi1r)
    if (Phi1r.extent(0) < num_nodes) Kokkos::realloc(Phi1r, num_nodes, num_lelm1lm2, num_channels);
    if (Phi1.extent(0) < num_nodes) Kokkos::realloc(Phi1, num_nodes, num_lme, num_channels);
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            ...
            auto Phi1r_i_lelm1lm2 =
                Kokkos::View<double*>(team_member.team_scratch(0), num_channels);
            Kokkos::parallel_for(Kokkos::TeamVectorRange(team_member, num_channels),
                                 [=](const int k) { Phi1r_i_lelm1lm2(k) = 0.0; });
            team_member.team_barrier();
            ...
            for (int j = 0; j < num_neigh(i); ++j) {
                ...
                Kokkos::parallel_for(
                    Kokkos::TeamVectorRange(team_member, num_channels), [=](const int k) {
                        Phi1r_i_lelm1lm2(k) += R1(ij, lel1l2 * num_channels + k) *
                                               Y(ij * num_lm + lm1) * H1(neigh_indices(ij), lm2, k);
                    });
            }
            team_member.team_barrier();
            // store Phi1r_i_lelm1lm2
            Kokkos::parallel_for(Kokkos::TeamVectorRange(team_member, num_channels),
                                 [=](const int k) { Phi1r(i, lelm1lm2, k) = Phi1r_i_lelm1lm2(k); });
        });
    Kokkos::fence();
    // #endif
    ...
    Kokkos::parallel_for(
        "Compute Phi1", Kokkos::TeamPolicy<>(num_nodes, Kokkos::AUTO, 32),
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            const int i = team_member.league_rank();
...
template <typename Precision>
void MACEKokkos<Precision>::reverse_Phi1(const int num_nodes, Kokkos::View<const int*> num_neigh,
                                         Kokkos::View<const int*> neigh_indices,
                                         Kokkos::View<const double*> xyz,
                                         Kokkos::View<const double*> r, bool zero_dxyz,
                                         bool zero_H1_adj) {
    if (dPhi1r.extent(0) < Phi1r.extent(0))
        Kokkos::realloc(dPhi1r, Phi1r.extent(0), Phi1r.extent(1), Phi1r.extent(2));
    if (node_forces.size() < xyz.size()) Kokkos::resize(node_forces, xyz.size());
    if (H1_adj.extent(0) < H1.extent(0))
        Kokkos::resize(H1_adj, H1.extent(0), H1.extent(1), H1.extent(2));
    if (zero_dxyz) Kokkos::deep_copy(node_forces, 0.0);
    Kokkos::deep_copy(dPhi1r, 0.0);
    if (zero_H1_adj) Kokkos::deep_copy(H1_adj, 0.0);
    ...
    Kokkos::parallel_for(
        "Reverse Phi1", Kokkos::TeamPolicy<>(num_nodes, Kokkos::AUTO, 32),
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            const int i = team_member.league_rank();
            for (int p = 0; p < Phi1_clebsch_gordan.size(); ++p) {
                const double C = Phi1_clebsch_gordan(p);
    ...
    Kokkos::parallel_for(
        "Reverse Phi1r", Kokkos::TeamPolicy<>(num_nodes, Kokkos::AUTO, 32),
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            const int i = team_member.league_rank();
            const int i0 = first_neigh(i);
            for (int j = 0; j < num_neigh(i); ++j) {
                ...
                Kokkos::parallel_reduce(
                    ...
                    [=](const int lelm1lm2, double& f_x, double& f_y, double& f_z) {
                        ...
                        Kokkos::parallel_reduce(
                            Kokkos::ThreadVectorRange(team_member, num_channels),
                            [=](const int k, double& t1, double& t2) {
                                t1 += R1_deriv(ij, lel1l2 * num_channels + k) *
                                      H1(neigh_indices(ij), lm2, k) * dPhi1r(i, lelm1lm2, k);
                                t2 += R1(ij, lel1l2 * num_channels + k) *
                                      H1(neigh_indices(ij), lm2, k) * dPhi1r(i, lelm1lm2, k);
                                Kokkos::atomic_add(&H1_adj(neigh_indices(ij), lm2, k),
                                                   R1(ij, lel1l2 * num_channels + k) *
                                                       Y(ij * num_lm + lm1) *
                                                       dPhi1r(i, lelm1lm2, k));
                            },
                            t1, t2);
                        f_x += t1 * xyz(3 * ij) / r(ij) * Y(ij * num_lm + lm1) +
                               t2 * Y_grad(3 * ij * num_lm + lm1);
                        f_y += t1 * xyz(3 * ij + 1) / r(ij) * Y(ij * num_lm + lm1) +
                               t2 * Y_grad((3 * ij + 1) * num_lm + lm1);
                        f_z += t1 * xyz(3 * ij + 2) / r(ij) * Y(ij * num_lm + lm1) +
                               t2 * Y_grad((3 * ij + 2) * num_lm + lm1);
                    },
                    f_x, f_y, f_z);
                team_member.team_barrier();
                Kokkos::single(Kokkos::PerTeam(team_member), [=]() {
...
void MACEKokkos<Precision>::compute_A1(int num_nodes) {
    ...
    if (A1.extent(0) < num_nodes) Kokkos::realloc(A1, num_nodes, num_lm, num_channels);
    ...
    Kokkos::parallel_for(
        "Compute A1", Kokkos::TeamPolicy<>(num_nodes * (l_max + 1), Kokkos::AUTO),
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            const int i = team_member.league_rank() / (l_max + 1);
            const int l = team_member.league_rank() % (l_max + 1);
            int lme = 0;
            ...
            auto A1_il =
                Kokkos::subview(A1, i, Kokkos::make_pair(l * l, l * (l + 2) + 1), Kokkos::ALL);
            KokkosBatched::TeamGemm<Kokkos::TeamPolicy<>::member_type,
                                    KokkosBatched::Trans::NoTranspose,
                                    KokkosBatched::Trans::NoTranspose,
...
void MACEKokkos<Precision>::reverse_A1(int num_nodes) {
    ...
    if (dPhi1.extent(0) < num_nodes) Kokkos::realloc(dPhi1, num_nodes, num_lme, num_channels);
    ...
    Kokkos::parallel_for(
        "Reverse A1", Kokkos::TeamPolicy<>(num_nodes * (l_max + 1), Kokkos::AUTO),
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            const int i = team_member.league_rank() / (l_max + 1);
            const int l = team_member.league_rank() % (l_max + 1);
            int lme = 0;
            ...
            auto dA1_il = Kokkos::subview(A1_adj, i, Kokkos::make_pair(l * l, l * l + 2 * l + 1),
                                          Kokkos::ALL);
...
template <typename Precision>
void MACEKokkos<Precision>::compute_A1_scaled(const int num_nodes,
                                              Kokkos::View<const int*> node_types,
                                              Kokkos::View<const int*> num_neigh,
                                              Kokkos::View<const int*> neigh_types,
                                              Kokkos::View<const double*> r) {
    ...
    A1_splines.evaluate(num_nodes, node_types, num_neigh, neigh_types, r, A1_spline_values,
                        A1_spline_derivs);
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            ...
            Kokkos::parallel_reduce(
                Kokkos::TeamThreadRange(team_member, num_neigh(i)),
                [=](const int j, double& lsum) { lsum += A1_spline_values(i0 + j, 0); },
                A1_scale_factor);
            A1_scale_factor += 1.0;
            team_member.team_barrier();
            // perform the scaling
            Kokkos::parallel_for(
                Kokkos::TeamThreadRange(team_member, num_lm * num_channels),
                [=](int lmk) { A1(i, lmk / num_channels, lmk % num_channels) /= A1_scale_factor; });
        });
    Kokkos::fence();
}

template <typename Precision>
void MACEKokkos<Precision>::reverse_A1_scaled(const int num_nodes,
                                              Kokkos::View<const int*> node_types,
                                              Kokkos::View<const int*> num_neigh,
                                              Kokkos::View<const int*> neigh_types,
                                              Kokkos::View<const double*> xyz,
        ...
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            ...
            Kokkos::parallel_for(Kokkos::TeamThreadRange(team_member, num_neigh(i)),
                                 [=](const int j) {
                                     ...
                                     node_forces(3 * ij + 0) +=
                                         dA1_dot_A1 / A1_scale_factor * d * xyz(3 * ij + 0) / r(ij);
                                     node_forces(3 * ij + 1) +=
                                         dA1_dot_A1 / A1_scale_factor * d * xyz(3 * ij + 1) / r(ij);
                                     node_forces(3 * ij + 2) +=
                                         dA1_dot_A1 / A1_scale_factor * d * xyz(3 * ij + 2) / r(ij);
                                 });
            // update dE/dA1
            Kokkos::parallel_for(
                Kokkos::TeamThreadRange(team_member, num_lm * num_channels), [=](int lmk) {
...
template <typename Precision>
void MACEKokkos<Precision>::compute_M1(int num_nodes, Kokkos::View<const int*> node_types) {
    if (M1.extent(0) < num_nodes) Kokkos::realloc(M1, num_nodes, num_channels);
    ...
    Kokkos::parallel_for(
        "Compute M1", Kokkos::TeamPolicy<>(num_nodes, Kokkos::AUTO, 32),
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            const int i = team_member.league_rank();
            // initialize
            Kokkos::parallel_for(
                Kokkos::TeamVectorMDRange<Kokkos::Rank<2, Kokkos::Iterate::Right>,
                                          Kokkos::TeamPolicy<>::member_type>(team_member, num_lm,
                                                                             num_channels),
                [&](const int p, const int k) {
                    M1_poly_values(i, p, k) = A1(i, p, k);
                    Kokkos::atomic_add(
                        &M1(i, k), M1_poly_coeff(node_types(i), p, k) * M1_poly_values(i, p, k));
                });
            team_member.team_barrier();
            // forward pass
            for (int p = 0; p < M1_poly_spec.extent(0); ++p) {
                ...
                Kokkos::parallel_for(Kokkos::TeamVectorRange(team_member, num_channels),
                                     [&](const int k) {
                                         M1_poly_values(i, num_lm + p, k) =
                                             M1_poly_values(i, p0, k) * M1_poly_values(i, p1, k);
                                         M1(i, k) += M1_poly_coeff(node_types(i), num_lm + p, k) *
                                                     M1_poly_values(i, num_lm + p, k);
                                     });
            }
        });
...
void MACEKokkos<Precision>::reverse_M1(int num_nodes, Kokkos::View<const int*> node_types) {
    ...
    Kokkos::parallel_for(
        "Reverse M1", Kokkos::TeamPolicy<>(num_nodes, Kokkos::AUTO, 32),
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            const int i = team_member.league_rank();
            // initialize
            Kokkos::parallel_for(Kokkos::TeamVectorMDRange<Kokkos::Rank<2, Kokkos::Iterate::Right>,
            ...
            for (int p = M1_poly_spec.extent(0) - 1; p >= 0; --p) {
                ...
                Kokkos::parallel_for(
                    Kokkos::TeamVectorRange(team_member, num_channels), [&](const int k) {
                        M1_poly_adjoints(i, p0, k) +=
                            M1_poly_adjoints(i, num_lm + p, k) * M1_poly_values(i, p1, k);
                        M1_poly_adjoints(i, p1, k) +=
                            M1_poly_adjoints(i, num_lm + p, k) * M1_poly_values(i, p0, k);
                    });
            }
            team_member.team_barrier();
            Kokkos::parallel_for(Kokkos::TeamVectorMDRange<Kokkos::Rank<2, Kokkos::Iterate::Right>,
...
void MACEKokkos<Precision>::compute_H2(int num_nodes, Kokkos::View<const int*> node_types) {
    ...
    Kokkos::parallel_for(
        "Compute H2 from H1", num_nodes * num_channels, KOKKOS_LAMBDA(const int ik) {
    ...
    Kokkos::fence();
    Kokkos::parallel_for(
        "Compute H2 from M1", num_nodes * num_channels, KOKKOS_LAMBDA(const int ik) {
            const int i = ik / num_channels;
            const int k = ik % num_channels;
            for (int kp = 0; kp < num_channels; ++kp) {
...
template <typename Precision>
void MACEKokkos<Precision>::reverse_H2(int num_nodes, Kokkos::View<const int*> node_types,
                                       bool zero_H1_adj) {
    if (H1_adj.extent(0) < H1.extent(0))
        Kokkos::resize(H1_adj, H1.extent(0), H1.extent(1), H1.extent(2));
    if (M1_adj.extent(0) < M1.extent(0)) Kokkos::realloc(M1_adj, M1.extent(0), M1.extent(1));
    ...
    Kokkos::parallel_for(
        "Reverse H2", num_nodes * num_channels, KOKKOS_LAMBDA(const int ik) {
            const int i = ik / num_channels;
            const int k = ik % num_channels;
            for (int kp = 0; kp < num_channels; ++kp) {
                H1_adj(i, 0, k) +=
                    H2_weights_for_H1(node_types(i), k * num_channels + kp) * H2_adj(i, kp);
...
template <typename Precision>
double MACEKokkos<Precision>::compute_readouts(int num_nodes,
    ...
    if (readout_2_output.extent(0) < num_nodes) Kokkos::realloc(readout_2_output, num_nodes);
    ...
    // atomic energies
    Kokkos::parallel_for(
        "Compute Readouts 1", num_nodes,
        KOKKOS_LAMBDA(const int i) { node_energies(i) += atomic_energies(node_types(i)); });
    Kokkos::fence();
    // first readout
    Kokkos::parallel_for(
        "Compute Readouts 1", num_nodes, KOKKOS_LAMBDA(const int i) {
    ...
    Kokkos::parallel_for(
        "Compute Readouts 2", num_nodes,
        KOKKOS_LAMBDA(const int i) { node_energies(i) += readout_2_output(i); });

    double energy;
    Kokkos::parallel_reduce(
        num_nodes,
        KOKKOS_LAMBDA(const int& i, double& local_sum) { local_sum += node_energies(i); }, energy);
...
void MACEKokkos<Precision>::load_from_json(std::string filename) {
    ...
    atomic_energies =
        toKokkosView("atomic_energies", file["atomic_energies"].get<std::vector<double>>());

    // ZBL
    has_zbl = file["has_zbl"].get<bool>();
    if (has_zbl)
        zbl = ZBLKokkos(file["zbl_a_exp"].get<double>(), file["zbl_a_prefactor"].get<double>(),
    ...
    auto c = Kokkos::View<Precision****, Kokkos::LayoutRight>(
        "c", atomic_numbers.size() * atomic_numbers.size(), spl_values_0[0][0].size() - 1, 4,
        (l_max + 1) * num_channels);
    ...
    for (int a = 0; a < atomic_numbers.size(); ++a) {
        for (int b = 0; b < atomic_numbers.size(); ++b) {
            ...
            const int ab_unordered = (a <= b) ? a * (2 * atomic_numbers.size() - a - 1) / 2 + b
            ...
            for (int i = 0; i < spl_values_0[0][0].size() - 1; ++i) {
                for (int lk = 0; lk < (l_max + 1) * num_channels; ++lk) {
                    ...
                    h_c(ab, i, 2, lk) =
                        (-3 * spl_values[lk][i] - 2 * spl_h * spl_derivs[lk][i] +
                         3 * spl_values[lk][i + 1] - spl_h * spl_derivs[lk][i + 1]) /
                        (spl_h * spl_h);
                    h_c(ab, i, 3, lk) =
                        (2 * spl_values[lk][i] + spl_h * spl_derivs[lk][i] -
                         2 * spl_values[lk][i + 1] + spl_h * spl_derivs[lk][i + 1]) /
                        (spl_h * spl_h * spl_h);
                }
            }
            // add H0_weights
            ...
            for (int i = 0; i < spl_values_0[0][0].size() - 1; ++i) {
                for (int l = 0; l <= l_max; ++l) {
                    auto c0 = std::vector<double>(&h_c(ab, i, 0, l * num_channels),
                                                  &h_c(ab, i, 0, l * num_channels) + num_channels);
                    auto c1 = std::vector<double>(&h_c(ab, i, 1, l * num_channels),
                                                  &h_c(ab, i, 1, l * num_channels) + num_channels);
                    auto c2 = std::vector<double>(&h_c(ab, i, 2, l * num_channels),
                                                  &h_c(ab, i, 2, l * num_channels) + num_channels);
                    auto c3 = std::vector<double>(&h_c(ab, i, 3, l * num_channels),
                                                  &h_c(ab, i, 3, l * num_channels) + num_channels);
                    for (int k = 0; k < num_channels; ++k) {
                        ...
                        for (int kp = 0; kp < num_channels; ++kp) {
                            h_c(ab, i, 0, l * num_channels + k) +=
                                A0_weights[a][l][kp * num_channels + k] * c0[kp];
                            h_c(ab, i, 1, l * num_channels + k) +=
                                A0_weights[a][l][kp * num_channels + k] * c1[kp];
                            h_c(ab, i, 2, l * num_channels + k) +=
                                A0_weights[a][l][kp * num_channels + k] * c2[kp];
                            h_c(ab, i, 3, l * num_channels + k) +=
                                A0_weights[a][l][kp * num_channels + k] * c3[kp];
                        }
                    }
                }
            }
    ...
    if (A0_scaled) {
        ...
        A0_splines =
            RadialFunctionSetKokkos<double>(A0_spline_h, A0_spline_values, A0_spline_derivs);
    }

    // M0 weights and monomials
    auto M0_weights_file =
        file["M0_weights"]
            .get<std::map<std::string,
                          std::map<std::string, std::map<std::string, std::vector<double>>>>>();
    auto M0_monomials_file =
        file["M0_monomials"].get<std::map<std::string, std::vector<std::vector<int>>>>();
    M0_weights =
        Kokkos::View<Kokkos::View<Precision***, Kokkos::LayoutRight>*, Kokkos::SharedSpace>(
    ...
    for (int LM = 0; LM < num_LM; ++LM) {
        M0_weights(LM) = Kokkos::View<Precision***, Kokkos::LayoutRight>(
            Kokkos::view_alloc(std::string("M0_weights_") + std::to_string(LM),
                               Kokkos::WithoutInitializing),
        ...
        M0_monomials(LM) = Kokkos::View<int**, Kokkos::LayoutRight>(
            Kokkos::view_alloc(std::string("M0_monomials_") + std::to_string(LM),
                               Kokkos::WithoutInitializing),
    ...
    for (int LM = 0; LM < num_LM; ++LM) {
        auto P = MultivariatePolynomial(
            num_lm, M0_weights_file[std::to_string(0)][std::to_string(LM)][std::to_string(0)],
            M0_monomials_file[std::to_string(LM)]);
        M0_poly_spec(LM) = Kokkos::View<int**, Kokkos::LayoutRight>(
            Kokkos::view_alloc(std::string("M0_poly_spec_") + std::to_string(LM),
                               Kokkos::WithoutInitializing),
            P.edges.size(), 2);
    ...
    for (int LM = 0; LM < num_LM; ++LM) {
        ...
        M0_poly_coeff(LM) = Kokkos::View<Precision***, Kokkos::LayoutRight>(
            Kokkos::view_alloc(std::string("M0_poly_coeff_") + std::to_string(LM),
                               Kokkos::WithoutInitializing),
    ...
    // H1 weights
    set_kokkos_view(H1_weights, file["H1_weights"].get<std::vector<Precision>>(), L_max + 1,
    ...
    Phi1_clebsch_gordan = toKokkosView("Phi1_clebsch_gordan",
                                       file["Phi1_clebsch_gordan"].get<std::vector<Precision>>());
    ...
    for (int i = 0; i < h_Phi1_l.size(); ++i) num_lme += 2 * h_Phi1_l(i) + 1;
    num_lelm1lm2 = 0;
    ...
    for (int l = 0; l <= l_max; ++l) {
        int num_eta = 0;
        auto h_Phi1_l = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), Phi1_l);
        for (int i = 0; i < h_Phi1_l.size(); ++i) num_eta += (h_Phi1_l(i) == l);
        A1_weights(l) = Kokkos::View<Precision**, Kokkos::LayoutRight>(
            Kokkos::view_alloc(std::string("A1_weights_") + std::to_string(l),
                               Kokkos::WithoutInitializing),
    ...
    if (A1_scaled) {
        ...
        A1_splines =
            RadialFunctionSetKokkos<double>(A1_spline_h, A1_spline_values, A1_spline_derivs);
    }
    ...
    for (int i = 0; i < num_terms; ++i)
        for (int j = 0; j < M1_monomials[i].size(); ++j) h_M1_monomials(i, j) = M1_monomials[i][j];
    ...
    for (int a = 0; a < atomic_numbers.size(); ++a) {
        for (int k = 0; k < num_channels; ++k) {
            P1.push_back(MultivariatePolynomial(
                num_lm, M1_weights[std::to_string(a)][std::to_string(k)], M1_monomials));
        }
    }
    // M1_poly_spec
    ...
    // M1_poly_coeff
    Kokkos::realloc(M1_poly_coeff, atomic_numbers.size(), num_lm + P1[0].edges.size(),
                    num_channels);
    ...
    H2_weights_for_H1 = Kokkos::View<double**, Kokkos::LayoutRight>(
    ...
    H2_weights_for_M1 =
        toKokkosView("H2_weights_for_M1", file["H2_weights_for_M1"].get<std::vector<double>>());

    // Readouts
    // WARNING!
    // hardcoded 16
    readout_1_weights =
        toKokkosView("readout_1_weights", file["readout_1_weights"].get<std::vector<double>>());
