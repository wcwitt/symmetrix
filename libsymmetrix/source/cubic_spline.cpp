...
CubicSpline::CubicSpline(double h, std::vector<double> nodal_values,
                         std::vector<double> nodal_derivs)
    : h(h), c(generate_coefficients(h, nodal_values, nodal_derivs)) {}

double CubicSpline::evaluate(double r) {
    if (r < 0 or r > h * c.size() / 4 or std::isnan(r))
        throw std::invalid_argument("Out of bounds in CubicSpline::evaluate. r=" +
                                    std::to_string(r));
...
std::tuple<double, double> CubicSpline::evaluate_deriv(double r) {
    if (r < 0 or r > h * c.size() / 4 or std::isnan(r))
        throw std::invalid_argument("Out of bounds in CubicSpline::evaluate_deriv. r=" +
                                    std::to_string(r));
    const int i = std::clamp(static_cast<int>(r / h), 0, static_cast<int>(c.size() / 4 - 1));
    const double x = r - h * i;
    const double xx = x * x;
...
std::tuple<double, double> CubicSpline::evaluate_deriv_divided(double r) {
    if (r <= 0 or r > h * c.size() / 4 or std::isnan(r))
        throw std::invalid_argument("Out of bounds in CubicSpline::evaluate_deriv_divided. r=" +
                                    std::to_string(r));
    const int i = std::clamp(static_cast<int>(r / h), 0, static_cast<int>(c.size() / 4 - 1));
    const double x = r - h * i;
    const double xx = x * x;
...
auto CubicSpline::generate_coefficients(double h, std::vector<double> nodal_values,
                                        std::vector<double> nodal_derivs) -> std::vector<double> {
    ...
    for (int i = 0; i < nodal_values.size() - 1; ++i) {
        ...
        c[4 * i + 2] = (-3 * c[4 * i] - 2 * h * c[4 * i + 1] + 3 * nodal_values[i + 1] -
                        h * nodal_derivs[i + 1]) /
                       (h * h);
        c[4 * i + 3] =
            (2 * c[4 * i] + h * c[4 * i + 1] - 2 * nodal_values[i + 1] + h * nodal_derivs[i + 1]) /
            (h * h * h);
    }
    return c;
}
