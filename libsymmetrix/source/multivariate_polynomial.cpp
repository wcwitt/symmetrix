...
MultivariatePolynomial::MultivariatePolynomial(int num_variables, std::vector<double> coefficients,
                                               std::vector<std::vector<int>> monomials)
    : num_variables(num_variables), coefficients(coefficients), monomials(monomials) {
    // TODO: sanitize input
    ...
    for (int i = 0; i < monomials.size(); ++i) map.insert({monomials[i], coefficients[i]});
    coefficients.clear();
    monomials.clear();
    for (auto [m, c] : map) {
    ...
    for (int i = 0; i < num_variables; ++i) node_set.insert({i});
    for (auto monomial : monomials) node_set.insert(monomial);
    ...
    for (auto node : node_set) {
        ...
        const int i0 =
            std::distance(nodes.begin(), std::find(nodes.begin(), nodes.end(), factors[0]));
        const int i1 =
            std::distance(nodes.begin(), std::find(nodes.begin(), nodes.end(), factors[1]));
...
auto MultivariatePolynomial::evaluate(const std::vector<double>& x) -> double {
    initialize_forward_pass(x);
    forward_pass();
    return cblas_ddot(node_coefficients.size(), node_coefficients.data(), 1, node_values.data(), 1);
}

auto MultivariatePolynomial::evaluate_gradient(const std::vector<double>& x)
    -> std::tuple<double, std::vector<double>> {
    initialize_forward_pass(x);
    forward_pass();
    auto f =
        cblas_ddot(node_coefficients.size(), node_coefficients.data(), 1, node_values.data(), 1);
...
auto MultivariatePolynomial::evaluate_batch(const std::vector<double>& x, const int batch_size)
...
void MultivariatePolynomial::initialize_forward_pass(const std::vector<double>& x) {
    for (int i = 0; i < num_variables; ++i) node_values[i] = x[i];
}
...
void MultivariatePolynomial::initialize_backward_pass() {
    cblas_dcopy(node_coefficients.size(), node_coefficients.data(), 1, node_adjoints.data(), 1);
}
...
auto MultivariatePolynomial::extract_gradient_from_graph() -> std::vector<double> {
    std::vector<double> g(num_variables, 0.0);
    for (int i = 0; i < num_variables; ++i) g[i] = node_adjoints[i];
    return g;
}

void MultivariatePolynomial::batched_initialize_forward_pass(const std::vector<double>& x,
...
void MultivariatePolynomial::batched_forward_pass(const int batch_size) {
    for (int i = 0; i < edges.size(); ++i) {
        const auto [i0, i1] = edges[i];
...
void MultivariatePolynomial::batched_initialize_backward_pass(const int batch_size) {
...
void MultivariatePolynomial::batched_backward_pass(const int batch_size) {
    for (int i = edges.size() - 1; i >= 0; --i) {
        const auto [i0, i1] = edges[i];
        ...
        for (int j = 0; j < batch_size; ++j) node_adj_0[j] += node_adj[j] * node_val_1[j];
        for (int j = 0; j < batch_size; ++j) node_adj_1[j] += node_adj[j] * node_val_0[j];
    }
}

auto MultivariatePolynomial::batched_extract_gradient_from_graph(const int batch_size)
    -> std::vector<double> {
...
auto MultivariatePolynomial::evaluate_gradient_simple(const std::vector<double>& x)
    -> std::tuple<double, std::vector<double>> {
    double f = 0.0;
    for (int i = 0; i < coefficients.size(); ++i) {
        double monomial = x[monomials[i][0]];
