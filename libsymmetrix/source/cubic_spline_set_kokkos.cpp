...
CubicSplineSetKokkos::CubicSplineSetKokkos(double h, std::vector<std::vector<double>> nodal_values,
    ...
    for (int i = 0; i < num_nodes - 1; ++i) {
        for (int j = 0; j < num_splines; ++j) {
            ...
            h_c(i, 2, j) = (-3 * nodal_values[j][i] - 2 * h * nodal_derivs[j][i] +
                            3 * nodal_values[j][i + 1] - h * nodal_derivs[j][i + 1]) /
                           (h * h);
            h_c(i, 3, j) = (2 * nodal_values[j][i] + h * nodal_derivs[j][i] -
                            2 * nodal_values[j][i + 1] + h * nodal_derivs[j][i + 1]) /
                           (h * h * h);
        }
    }
...
struct InitializeCoefficientsFunctor {
    ...
    InitializeCoefficientsFunctor(double h_, Kokkos::View<double**> nodal_values_,
                                  Kokkos::View<double**> nodal_derivs_,
                                  Kokkos::View<double***, Kokkos::LayoutRight> c_)
    ...
    void operator()(const int i, const int j) const {
        ...
        c(i, 2, j) = (-3 * nodal_values(j, i) - 2 * h * nodal_derivs(j, i) +
                      3 * nodal_values(j, i + 1) - h * nodal_derivs(j, i + 1)) /
                     (h * h);
        c(i, 3, j) = (2 * nodal_values(j, i) + h * nodal_derivs(j, i) - 2 * nodal_values(j, i + 1) +
                      h * nodal_derivs(j, i + 1)) /
                     (h * h * h);
    }
};

CubicSplineSetKokkos::CubicSplineSetKokkos(double h, Kokkos::View<double**> nodal_values,
                                           Kokkos::View<double**> nodal_derivs) {
    // TODO: sanitize input
    this->h = h;
    ...
    Kokkos::parallel_for(
        ...
        Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {num_nodes - 1, num_splines}), functor);
    // Kokkos::fence();
}

void CubicSplineSetKokkos::evaluate(double r, Kokkos::View<double*> values) {
    ...
    Kokkos::parallel_for(
        "EvaluateSpline", num_splines, KOKKOS_CLASS_LAMBDA(const int j) {
            values(j) = c(i, 0, j) + c(i, 1, j) * x + c(i, 2, j) * xx + c(i, 3, j) * xxx;
        });
    Kokkos::fence();
}

void CubicSplineSetKokkos::evaluate_derivs(double r, Kokkos::View<double*> values,
    ...
    Kokkos::parallel_for(
        "evaluate_derivs", num_splines, KOKKOS_CLASS_LAMBDA(const int j) {
...
void CubicSplineSetKokkos::evaluate_derivs(
    Kokkos::View<const double*> r, Kokkos::View<double**, Kokkos::LayoutRight> values,
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            ...
            const double three_xx = 3 * xx;
            // compute values
            Kokkos::parallel_for(Kokkos::ThreadVectorRange(team_member, num_splines),
                                 [&](const int j) { values(i, j) = c(ii, 0, j); });
            team_member.team_barrier();
            Kokkos::parallel_for(Kokkos::ThreadVectorRange(team_member, num_splines),
                                 [&](const int j) { values(i, j) += c(ii, 1, j) * x; });
            team_member.team_barrier();
            Kokkos::parallel_for(Kokkos::ThreadVectorRange(team_member, num_splines),
                                 [&](const int j) { values(i, j) += c(ii, 2, j) * xx; });
            team_member.team_barrier();
            Kokkos::parallel_for(Kokkos::ThreadVectorRange(team_member, num_splines),
                                 [&](const int j) { values(i, j) += c(ii, 3, j) * xxx; });
            // compute derivs
            Kokkos::parallel_for(Kokkos::ThreadVectorRange(team_member, num_splines),
                                 [&](const int j) { derivs(i, j) = c(ii, 1, j); });
            team_member.team_barrier();
            Kokkos::parallel_for(Kokkos::ThreadVectorRange(team_member, num_splines),
                                 [&](const int j) { derivs(i, j) += c(ii, 2, j) * two_x; });
            team_member.team_barrier();
            Kokkos::parallel_for(Kokkos::ThreadVectorRange(team_member, num_splines),
                                 [&](const int j) { derivs(i, j) += c(ii, 3, j) * three_xx; });
        });
}

void CubicSplineSetKokkos::evaluate(double r, std::span<double> values) {
    ...
    Kokkos::View<double*, Kokkos::MemoryTraits<Kokkos::Unmanaged>> input_value_view(values.data(),
                                                                                    values.size());
...
void CubicSplineSetKokkos::evaluate_derivs(double r, std::span<double> values,
    ...
    // view to copy the values
    Kokkos::View<double*, Kokkos::MemoryTraits<Kokkos::Unmanaged>> input_values_view(values.data(),
                                                                                     values.size());
    Kokkos::View<double*, Kokkos::MemoryTraits<Kokkos::Unmanaged>> input_derivs_view(derivs.data(),
                                                                                     derivs.size());
    ...
    evaluate_derivs(r, test_view_values, test_view_derivs);
