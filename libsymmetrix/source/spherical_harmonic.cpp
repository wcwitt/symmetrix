...
std::complex<double> sph_harm(int l, int m, double polar, double azimuth) {
    if (m >= 0) {
        return std::pow(-1, m) *
               std::sqrt((2 * l + 1) / (4 * M_PI) * std::tgamma(1 + l - m) /
                         std::tgamma(1 + l + m)) *
        ...
        return std::pow(-1, m) *
               std::sqrt((2 * l + 1) / (4 * M_PI) * std::tgamma(1 + l - m) /
                         std::tgamma(1 + l + m)) *
               std::pow(-1, m) * std::tgamma(1 + l + m) / std::tgamma(1 + l - m) *
               std::assoc_legendre(l, -m, std::cos(polar)) *
               std::exp(std::complex<double>(0, 1) * double(m) * azimuth);
    }
}
...
std::vector<std::complex<double>> sphericart_complex(int l_max, std::vector<double> xyz) {
