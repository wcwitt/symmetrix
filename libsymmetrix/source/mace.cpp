...
MACE::MACE(std::string filename) { load_from_json(filename); }

void MACE::compute_node_energies_forces(const int num_nodes, std::span<const int> node_types,
...
void MACE::compute_R0(const int num_nodes, std::span<const int> node_types,
    ...
    for (int i = 0; i < num_nodes; ++i) {
        ...
        for (int j = 0; j < num_neigh[i]; ++j) {
            ...
            const int type_ij =
                (type_i <= type_j) ? type_i * (2 * atomic_numbers.size() - type_i - 1) / 2 + type_j
...
void MACE::compute_R1(const int num_nodes, std::span<const int> node_types,
                      std::span<const int> num_neigh, std::span<const int> neigh_types,
                      std::span<const double> r) {
...
void MACE::compute_Y(std::span<const double> xyz) {
    ...
    for (int i = 0; i < Y.size(); ++i) Y[i] *= 2 * std::sqrt(std::numbers::pi);
    for (int i = 0; i < Y_grad.size(); ++i) Y_grad[i] *= 2 * std::sqrt(std::numbers::pi);
    // unshuffle gradient
...
void MACE::compute_A0(const int num_nodes, std::span<const int> node_types,
                      std::span<const int> num_neigh, std::span<const int> neigh_types) {
...
void MACE::reverse_A0(const int num_nodes, std::span<const int> node_types,
                      std::span<const int> num_neigh, std::span<const int> neigh_types,
                      std::span<const double> xyz, std::span<const double> r) {
    int ij = 0;
    for (int i = 0; i < num_nodes; ++i) {
        ...
        for (int j = 0; j < num_neigh[i]; ++j) {
            ...
            for (int l = 0; l <= l_max; ++l) {
                auto R0_ij_l = R0.data() + ij * (l_max + 1) * num_channels + l * num_channels;
                auto R0_deriv_ij_l =
                    R0_deriv.data() + ij * (l_max + 1) * num_channels + l * num_channels;
                for (int m = -l; m <= l; ++m) {
                    const int lm = l * l + l + m;
                    const double Y_ij_lm = Y_ij[lm];
...
void MACE::compute_A0_scaled(const int num_nodes, std::span<const int> node_types,
                             std::span<const int> num_neigh, std::span<const int> neigh_types,
                             std::span<const double> r) {
    ...
    for (int i = 0; i < num_nodes; ++i) {
        ...
        for (int lmk = 0; lmk < num_lm * num_channels; ++lmk) A0_i[lmk] /= A0_scale_factor;
    }
}

void MACE::reverse_A0_scaled(const int num_nodes, std::span<const int> node_types,
                             std::span<const int> num_neigh, std::span<const int> neigh_types,
                             std::span<const double> xyz, std::span<const double> r) {
    if (not A0_scaled) return;
    // Warning: Assumes node_forces have been initialized elsewhere
    ...
    for (int i = 0; i < num_nodes; ++i) {
        ...
        // update dE/dA0
        for (int lmk = 0; lmk < num_lm * num_channels; ++lmk) A0_adj_i[lmk] /= A0_scale_factor;
    }
}

void MACE::compute_M0(const int num_nodes, std::span<const int> node_types) {
    ...
    for (int i = 0; i < num_nodes; ++i) {
        ...
        for (int lm = 0; lm < num_LM; ++lm) {
            for (int k = 0; k < num_channels; ++k) {
                ...
                M0_i[lmk] = f;
                cblas_dcopy(num_lm, g.data(), 1, M0_grad_i + lm * num_lm * num_channels + k,
                            num_channels);
                lmk += 1;
            }
        }
...
void MACE::reverse_M0(const int num_nodes, std::span<const int> node_types) {
    ...
    for (int i = 0; i < num_nodes; ++i) {
        ...
        for (int lm = 0; lm < num_lm; ++lm) {
            ...
            for (int lmp = 0; lmp < num_LM; ++lmp) {
                ...
                auto M0_grad_ilmplm = M0_grad_i + +lmp * num_lm * num_channels + lm * num_channels;
                for (int k = 0; k < num_channels; ++k) {
...
void MACE::compute_H1(const int num_nodes) {
    H1.resize(M0.size());
    for (int i = 0; i < num_nodes; ++i) {
        for (int l = 0; l <= L_max; ++l) {
...
void MACE::reverse_H1(const int num_nodes) {
...
void MACE::compute_Phi1(const int num_nodes, std::span<const int> num_neigh,
                        std::span<const int> neigh_indices) {
    ...
    for (int i = 0; i < num_nodes; ++i) {
        ...
        for (int p = 0; p < Phi1_clebsch_gordan.size(); ++p) {
            ...
            for (int k = 0; k < num_channels; ++k) Phi1_i_lme[k] += C * Phi1r_i_lelm1lm2[k];
        }
    }
}

void MACE::reverse_Phi1(const int num_nodes, std::span<const int> num_neigh,
                        std::span<const int> neigh_indices, std::span<const double> xyz,
                        std::span<const double> r, bool zero_dxyz, bool zero_H1_adj) {
    ...
    for (int i = 0; i < num_nodes; ++i) {
        ...
        for (int p = 0; p < Phi1_clebsch_gordan.size(); ++p) {
            ...
            for (int k = 0; k < num_channels; ++k) dPhi1r_i_lelm1lm2[k] += C * dPhi1_i_lme[k];
        }
    }
    // Compute partial forces
    node_forces.resize(xyz.size());
    if (zero_dxyz) std::fill(node_forces.begin(), node_forces.end(), 0.0);
    int ij = 0;
    for (int i = 0; i < num_nodes; ++i) {
        auto dPhi1r_i = dPhi1r.data() + i * num_lelm1lm2 * num_channels;
        for (int j = 0; j < num_neigh[i]; ++j) {
    ...
    if (zero_H1_adj) std::fill(H1_adj.begin(), H1_adj.end(), 0.0);
    ij = 0;
    for (int i = 0; i < num_nodes; ++i) {
        auto dPhi1r_i = dPhi1r.data() + i * num_lelm1lm2 * num_channels;
        for (int j = 0; j < num_neigh[i]; ++j) {
...
void MACE::compute_A1(const int num_nodes) {
...
void MACE::reverse_A1(const int num_nodes) {
    // The core matrix multiplication is:
...
void MACE::compute_A1_scaled(const int num_nodes, std::span<const int> node_types,
                             std::span<const int> num_neigh, std::span<const int> neigh_types,
                             std::span<const double> r) {
    ...
    for (int i = 0; i < num_nodes; ++i) {
        ...
        for (int lmk = 0; lmk < num_lm * num_channels; ++lmk) A1_i[lmk] /= A1_scale_factor;
    }
}

void MACE::reverse_A1_scaled(const int num_nodes, std::span<const int> node_types,
                             std::span<const int> num_neigh, std::span<const int> neigh_types,
                             std::span<const double> xyz, std::span<const double> r,
                             bool zero_dxyz) {
    if (not A1_scaled) return;
    ...
    for (int i = 0; i < num_nodes; ++i) {
        ...
        // update dE/dA1
        for (int lmk = 0; lmk < num_lm * num_channels; ++lmk) A1_adj_i[lmk] /= A1_scale_factor;
    }
}

void MACE::compute_M1(const int num_nodes, std::span<const int> node_types) {
...
void MACE::reverse_M1(const int num_nodes, std::span<const int> node_types) {
...
void MACE::compute_H2(const int num_nodes, std::span<const int> node_types) {
    H2.resize(num_nodes * num_channels);
    for (int i = 0; i < num_nodes; ++i) {
        auto H2_i = H2.data() + i * num_channels;
        auto H1_i = H1.data() + i * num_LM * num_channels;
        cblas_dgemv(CblasRowMajor,                            // const CBLAS_LAYOUT Layout
...
void MACE::reverse_H2(const int num_nodes, std::span<const int> node_types, bool zero_H1_adj) {
    H1_adj.resize(H1.size());
...
void MACE::compute_readouts(const int num_nodes, std::span<const int> node_types) {
    node_energies.resize(num_nodes);
    H1_adj.resize(H1.size());
    ...
    for (int i = 0; i < num_nodes; ++i) {
        ...
        // second readout
        auto x =
            std::vector<double>(H2.begin() + i * num_channels, H2.begin() + (i + 1) * num_channels);
...
void MACE::load_from_json(const std::string filename) {
    ...
    if (has_zbl)
        zbl = ZBL(file["zbl_a_exp"].get<double>(), file["zbl_a_prefactor"].get<double>(),
                  file["zbl_c"].get<std::vector<double>>(),
                  file["zbl_covalent_radii"].get<std::vector<double>>(), file["zbl_p"].get<int>());
    ...
    for (int i = 0; i < spl_values_0.size(); ++i)
        spl_set_0.push_back(
            std::make_unique<CubicSplineSet>(spl_h, spl_values_0[i], spl_derivs_0[i]));
    auto spl_values_1 =
    ...
    for (int i = 0; i < spl_values_1.size(); ++i)
        spl_set_1.push_back(
            std::make_unique<CubicSplineSet>(spl_h, spl_values_1[i], spl_derivs_1[i]));
    ...
    if (A0_scaled) {
        ...
        for (int i = 0; i < A0_spline_values.size(); ++i)
            A0_splines.push_back(
                CubicSpline(A0_spline_h, A0_spline_values[i], A0_spline_derivs[i]));
    }
        ...
        for (int lm = 0; lm < num_LM; ++lm) {
            for (int k = 0; k < num_channels; ++k) {
                P0.push_back(MultivariatePolynomial(
                    num_lm, M0_weights[std::to_string(a)][std::to_string(lm)][std::to_string(k)],
    ...
    num_lme = 0;
    for (auto l : Phi1_l) num_lme += 2 * l + 1;
    num_lelm1lm2 = 0;
    ...
    if (A1_scaled) {
        ...
        for (int i = 0; i < A1_spline_values.size(); ++i)
            A1_splines.push_back(
                CubicSpline(A1_spline_h, A1_spline_values[i], A1_spline_derivs[i]));
    }
