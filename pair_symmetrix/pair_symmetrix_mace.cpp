...
PairSymmetrixMACE::PairSymmetrixMACE(LAMMPS *lmp) : Pair(lmp) {
    single_enable = 0;
    restartinfo = 0;
    one_coeff = 1;
    manybody_flag = 1;
...
void PairSymmetrixMACE::coeff(int narg, char **arg) {
    ...
    for (int i = 3; i < narg; ++i) {
        ...
        auto iter2 =
            std::find(mace->atomic_numbers.begin(), mace->atomic_numbers.end(), atomic_number);
...
void PairSymmetrixMACE::init_style() {
    if (atom->map_user == atom->MAP_NONE)
        error->all(FLERR, "symmetrix/mace requires \'atom_modify map [yes|array|hash]\'");
    ...
    if (mode == "no_domain_decomposition" or mode == "mpi_message_passing") {
        ...
        if (comm->get_comm_cutoff() < (2 * mace->r_cut + neighbor->skin)) {
            ...
            if (comm->me == 0)
                utils::logmesg(lmp, "symmetrix/mace is setting the communication cutoff to {}",
                               cutoff_val);
        }
...
int PairSymmetrixMACE::pack_forward_comm(int n, int *list, double *buf, int /*pbc_flag*/,
                                         int * /*pbc*/) {
    for (int ii = 0; ii < n; ++ii) {
        const int i = list[ii];
        for (int k = 0; k < mace->num_LM * mace->num_channels; ++k) {
            buf[ii * mace->num_LM * mace->num_channels + k] =
                H1[i * mace->num_LM * mace->num_channels + k];
        }
    }
...
void PairSymmetrixMACE::unpack_forward_comm(int n, int first, double *buf) {
    for (int i = 0; i < n; ++i) {
        for (int k = 0; k < mace->num_LM * mace->num_channels; ++k) {
            H1[(first + i) * mace->num_LM * mace->num_channels + k] =
                buf[i * mace->num_LM * mace->num_channels + k];
        }
    }
}
...
int PairSymmetrixMACE::pack_reverse_comm(int n, int first, double *buf) {
    for (int i = 0; i < n; ++i) {
        for (int k = 0; k < mace->num_LM * mace->num_channels; ++k) {
            buf[i * mace->num_LM * mace->num_channels + k] =
                H1_adj[(first + i) * mace->num_LM * mace->num_channels + k];
        }
    }
    return n * mace->num_LM * mace->num_channels;
}
...
void PairSymmetrixMACE::unpack_reverse_comm(int n, int *list, double *buf) {
    for (int ii = 0; ii < n; ++ii) {
        const int i = list[ii];
        for (int k = 0; k < mace->num_LM * mace->num_channels; ++k) {
            H1_adj[i * mace->num_LM * mace->num_channels + k] +=
                buf[ii * mace->num_LM * mace->num_channels + k];
        }
    }
}
...
void PairSymmetrixMACE::compute_no_domain_decomposition(int eflag, int vflag) {
    ...
    for (int ii = 0; ii < list->inum; ++ii) {
        ...
        for (int jj = 0; jj < list->numneigh[i]; jj++) {
            ...
            if (r_squared < r_cut_squared) num_edges += 1;
        }
    }
    ...
    if (eflag_global) {
        for (int ii = 0; ii < num_nodes; ++ii) eng_vdwl += mace->node_energies[ii];
    }

    if (eflag_atom) {
        for (int ii = 0; ii < num_nodes; ++ii) eatom[ii] = mace->node_energies[ii];
    }
...
void PairSymmetrixMACE::compute_mpi_message_passing(int eflag, int vflag) {
    ...
    for (int ii = 0; ii < list->inum; ++ii) {
        ...
        for (int k = 0; k < mace->num_LM * mace->num_channels; ++k) {
            H1[i * mace->num_LM * mace->num_channels + k] =
                mace->H1[ii * mace->num_LM * mace->num_channels + k];
        }
    }
...
void PairSymmetrixMACE::compute_no_mpi_message_passing(int eflag, int vflag) {
    ...
    for (int ii = 0; ii < list->inum; ++ii) {
        ...
        for (int jj = 0; jj < list->numneigh[i]; jj++) {
            ...
            const double dz = atom->x[j][2] - z_i;
            const double r_squared = dx * dx + dy * dy + dz * dz;
            if (r_squared < r_cut_squared and not is_local[j]) is_ghost[j] = true;
        }
    }
    ...
    for (int ii = 0; ii < atom->nlocal + atom->nghost; ++ii)
        if (is_ghost[ii]) ghost_indices[i++] = ii;
    ...
    for (int ii = 0; ii < num_local_nodes + num_ghost_nodes; ii++) {
        const int i =
            (ii < num_local_nodes) ? list->ilist[ii] : ghost_indices[ii - num_local_nodes];
        node_i[ii] = i;
        ii_from_i[i] = ii;
        node_types[ii] = mace_types[atom->type[i] - 1];
        const double x_i = atom->x[i][0];
        ...
        for (int jj = 0; jj < list->numneigh[i]; jj++) {
            ...
            const double dz = atom->x[j][2] - z_i;
            const double r_squared = dx * dx + dy * dy + dz * dz;
            if (r_squared < r_cut_squared) num_neigh[ii] += 1;
        }
    }

    // count edges
    int num_local_edges = 0;
    for (int ii = 0; ii < num_local_nodes; ++ii) num_local_edges += num_neigh[ii];
    ...
    if (eflag_global)
        for (int ii = 0; ii < num_local_nodes; ++ii) eng_vdwl += mace->node_energies[ii];

    if (eflag_atom)
        for (int ii = 0; ii < num_local_nodes; ++ii) eatom[ii] = mace->node_energies[ii];
