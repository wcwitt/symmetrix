...
PairSymmetrixMACEKokkos<DeviceType, Precision>::PairSymmetrixMACEKokkos(LAMMPS* lmp) : Pair(lmp) {
    single_enable = 0;
    restartinfo = 0;
    one_coeff = 1;
    manybody_flag = 1;
...
void PairSymmetrixMACEKokkos<DeviceType, Precision>::allocate() {
    ...
    for (int i = 1; i < atom->ntypes + 1; ++i)
        for (int j = i; j < atom->ntypes + 1; ++j) setflag[i][j] = 0;
...
void PairSymmetrixMACEKokkos<DeviceType, Precision>::settings(int narg, char** arg) {
    if (narg == 0) {
        ...
        if (mode != "no_domain_decomposition" and mode != "mpi_message_passing" and
            mode != "no_mpi_message_passing")
...
void PairSymmetrixMACEKokkos<DeviceType, Precision>::coeff(int narg, char** arg) {
    ...
    for (int i = 1; i < atom->ntypes + 1; i++)
        for (int j = i; j < atom->ntypes + 1; j++) setflag[i][j] = 1;
}
...
void PairSymmetrixMACEKokkos<DeviceType, Precision>::init_style() {
    if (atom->map_user == atom->MAP_NONE)
        error->all(FLERR, "symmetrix/mace/kk requires \'atom_modify map [yes|array|hash]\'");
    ...
    if (mode == "no_domain_decomposition" or mode == "mpi_message_passing") {
        ...
        if (comm->get_comm_cutoff() < (2 * mace->r_cut + neighbor->skin)) {
            ...
            if (comm->me == 0)
                error->warning(FLERR, "symmetrix/mace/kk is setting the communication cutoff to {}",
                               cutoff_val);
        }
...
template <class DeviceType, typename Precision>
int PairSymmetrixMACEKokkos<DeviceType, Precision>::pack_forward_comm(int n, int* list, double* buf,
                                                                      int /*pbc_flag*/,
                                                                      int* /*pbc*/) {
    ...
    for (int ii = 0; ii < n; ++ii) {
        ...
        for (int LM = 0; LM < mace->num_LM; ++LM) {
            for (int k = 0; k < mace->num_channels; ++k) {
                buf[ii * mace->num_LM * mace->num_channels + LM * mace->num_channels + k] =
                    h_H1(i, LM, k);
            }
        }
    }
...
int PairSymmetrixMACEKokkos<DeviceType, Precision>::pack_forward_comm_kokkos(
    int n, DAT::tdual_int_1d k_sendlist, DAT::tdual_double_1d& buf, int /*pbc_flag*/,
    int* /*pbc*/) {
...
template <class DeviceType, typename Precision>
void PairSymmetrixMACEKokkos<DeviceType, Precision>::unpack_forward_comm(int n, int first,
                                                                         double* buf) {
    ...
    for (int i = 0; i < n; ++i) {
        for (int LM = 0; LM < mace->num_LM; ++LM) {
            for (int k = 0; k < mace->num_channels; ++k) {
                h_H1((first + i), LM, k) =
                    buf[i * mace->num_LM * mace->num_channels + LM * mace->num_channels + k];
            }
        }
    }
...
template <class DeviceType, typename Precision>
int PairSymmetrixMACEKokkos<DeviceType, Precision>::pack_reverse_comm(int n, int first,
                                                                      double* buf) {
...
template <class DeviceType, typename Precision>
void PairSymmetrixMACEKokkos<DeviceType, Precision>::unpack_reverse_comm(int n, int* list,
                                                                         double* buf) {
...
void PairSymmetrixMACEKokkos<DeviceType, Precision>::unpack_reverse_comm_kokkos(
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(const int ii, const int LM, const int k) {
            const int i = d_sendlist(ii);
            Kokkos::atomic_add(&H1_adj(i, LM, k),
...
template <class DeviceType, typename Precision>
void PairSymmetrixMACEKokkos<DeviceType, Precision>::compute_no_domain_decomposition(int eflag,
                                                                                     int vflag) {
    ...
    Kokkos::parallel_for(
        "PairSymmetrixMACEKokkos::set_node_based_views",
    ...
    int num_edges;
    Kokkos::parallel_reduce(
        "PairSymmetrixMACEKokkos::count_edges", num_nodes,
    ...
    Kokkos::parallel_scan(
        "PairSymmetrixMACEKokkos::populate_first_neigh", num_nodes,
    ...
    Kokkos::parallel_for(
        "PairSymmetrixMACEKokkos::set_edge_based_views", num_nodes, KOKKOS_LAMBDA(const int ii) {
            const int i = d_ilist(ii);
            const double x_i = x(i, 0);
            const double y_i = x(i, 1);
            const double z_i = x(i, 2);
    ...
    mace->compute_node_energies_forces(num_nodes, node_types, num_neigh, neigh_indices, neigh_types,
                                       xyz, r);
    ...
    if (eflag_global) {
        ...
        double energy;
        Kokkos::parallel_reduce(
            "PairSymmetrixMACEKokkos::energy_reduction", num_nodes,
            KOKKOS_LAMBDA(const int i, double& energy) { energy += node_energies(i); }, energy);
        eng_vdwl += energy;
    }
    ...
    if (eflag_atom) {
        ...
        auto node_energies = mace->node_energies;
        Kokkos::parallel_for(
            "PairSymmetrixMACEKokkos::extract_atomic_energies", num_nodes,
            KOKKOS_LAMBDA(const int ii) { d_eatom(ii) += node_energies(ii); });
        k_eatom.modify<DeviceType>();
    }

    auto mace_node_forces = mace->node_forces;
    Kokkos::parallel_for(
        "PairSymmetrixMACEKokkos::force_reduction", Kokkos::TeamPolicy<>(num_nodes, Kokkos::AUTO),
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            const int ii = team_member.league_rank();
    ...
    if (vflag_global) {
        ...
        Kokkos::parallel_for(
            "PairSymmetrixMACEKokkos::virial_reduction",
            Kokkos::TeamPolicy<>(num_nodes, Kokkos::AUTO),
            KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
                const int ii = team_member.league_rank();
                const int i = node_indices(ii);
                ...
                Kokkos::parallel_reduce(
                    Kokkos::TeamThreadRange(team_member, num_neigh(ii)),
                    [&](const int jj, double& v_0, double& v_1, double& v_2, double& v_3,
                        double& v_4, double& v_5) {
                        const int ij = first_neigh(ii) + jj;
...
template <class DeviceType, typename Precision>
void PairSymmetrixMACEKokkos<DeviceType, Precision>::compute_mpi_message_passing(int eflag,
                                                                                 int vflag) {
    ev_init(eflag, vflag, 0);
    ...
    auto mace_types = this->mace_types;
    Kokkos::parallel_for(
        "Set Node-Based Views", Kokkos::TeamPolicy<>(num_nodes, Kokkos::AUTO),
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            const int ii = team_member.league_rank();
            const int i = d_ilist(ii);
            node_indices(ii) = i;
    ...
    auto r = Kokkos::subview(this->r, Kokkos::make_pair(0, num_edges));
    Kokkos::parallel_for(
        "Set Edge-Based Views", num_nodes, KOKKOS_LAMBDA(const int ii) {
            const int i = d_ilist(ii);
            const double x_i = x(i, 0);
            const double y_i = x(i, 1);
            const double z_i = x(i, 2);
    ...
    if (mace->has_zbl)
        mace->zbl.compute_ZBL(num_nodes, node_types, num_neigh, neigh_types, mace->atomic_numbers,
                              r, xyz, mace->node_energies, mace->node_forces);
    ...
    if (eflag_atom) {
        ...
        auto node_energies = mace->node_energies;
        Kokkos::parallel_for(
            "Extract Atomic Energies", num_nodes,
            KOKKOS_LAMBDA(const int ii) { d_eatom(ii) += node_energies(ii); });
        k_eatom.modify<DeviceType>();
    }
    ...
    auto mace_node_forces = mace->node_forces;
    Kokkos::parallel_for(
        "Force Reduction", Kokkos::TeamPolicy<>(num_nodes, Kokkos::AUTO),
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            const int ii = team_member.league_rank();
            const int i = node_indices(ii);
            double f_x, f_y, f_z;
    ...
    if (vflag_global) {
        ...
        Kokkos::deep_copy(v, 0.0);
        Kokkos::parallel_for(
            "Virial Reduction", Kokkos::TeamPolicy<>(num_nodes, Kokkos::AUTO),
            KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
                const int ii = team_member.league_rank();
                const int i = node_indices(ii);
                double v_0, v_1, v_2, v_3, v_4, v_5;
...
template <class DeviceType, typename Precision>
void PairSymmetrixMACEKokkos<DeviceType, Precision>::compute_no_mpi_message_passing(int eflag,
                                                                                    int vflag) {
    ev_init(eflag, vflag, 0);
    ...
    Kokkos::parallel_for(
        "fill is_local", list->inum, KOKKOS_LAMBDA(const int ii) {
            const int i = d_ilist(ii);
            is_local.set(i);
        });
    Kokkos::fence();
    ...
    Kokkos::parallel_for(
        "fill is_ghost", Kokkos::TeamPolicy<>(list->inum, Kokkos::AUTO),
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            const int ii = team_member.league_rank();
            const int i = d_ilist(ii);
            const double x_i = x(i, 0);
            const double y_i = x(i, 1);
            const double z_i = x(i, 2);
            Kokkos::parallel_for(Kokkos::TeamThreadRange(team_member, d_numneigh(i)),
                                 [&](const int jj) {
                                     const int j = (d_neighbors(i, jj) & NEIGHMASK);
                                     const double dx = x(j, 0) - x_i;
                                     const double dy = x(j, 1) - y_i;
    ...
    Kokkos::parallel_scan(
        "populate ghost_indices", is_ghost.size(),
        KOKKOS_LAMBDA(int i, int& update, const bool final) {
            if (final && is_ghost.test(i)) ghost_indices(update) = i;
    ...
    auto node_indices = Kokkos::subview(this->node_indices,
                                        Kokkos::make_pair(0, num_local_nodes + num_ghost_nodes));
    auto node_types =
        Kokkos::subview(this->node_types, Kokkos::make_pair(0, num_local_nodes + num_ghost_nodes));
    auto num_neigh =
        Kokkos::subview(this->num_neigh, Kokkos::make_pair(0, num_local_nodes + num_ghost_nodes));
    Kokkos::deep_copy(num_neigh, 0);
    auto mace_types = this->mace_types;
    Kokkos::parallel_for(
        "populate node-based views",
        Kokkos::TeamPolicy<>(num_local_nodes + num_ghost_nodes, Kokkos::AUTO),
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            const int ii = team_member.league_rank();
            const int i =
                (ii < num_local_nodes) ? d_ilist(ii) : ghost_indices(ii - num_local_nodes);
            node_indices(ii) = i;
            node_types(ii) = mace_types(type(i) - 1);
            const double x_i = x(i, 0);
            const double y_i = x(i, 1);
    ...
    int num_local_edges;
    Kokkos::parallel_reduce(
        "count local edges", num_local_nodes,
        KOKKOS_LAMBDA(const int ii, int& num_local_edges) { num_local_edges += num_neigh(ii); },
        num_local_edges);
    int num_ghost_edges;
    Kokkos::parallel_reduce(
        "count ghost edges",
        Kokkos::RangePolicy<>(num_local_nodes, num_local_nodes + num_ghost_nodes),
        KOKKOS_LAMBDA(const int ii, int& num_ghost_edges) { num_ghost_edges += num_neigh(ii); },
        num_ghost_edges);
    Kokkos::fence();
    ...
    auto first_neigh =
        Kokkos::subview(this->first_neigh, Kokkos::make_pair(0, num_local_nodes + num_ghost_nodes));
    Kokkos::parallel_scan(
        "populate first neighbor", num_local_nodes + num_ghost_nodes,
        KOKKOS_LAMBDA(const int ii, int& first_neigh_ii, const bool final) {
            if (final) first_neigh(ii) = first_neigh_ii;
            first_neigh_ii += num_neigh(ii);
        });
    ...
    auto neigh_indices = Kokkos::subview(this->neigh_indices,
                                         Kokkos::make_pair(0, num_local_edges + num_ghost_edges));
    auto neigh_types =
        Kokkos::subview(this->neigh_types, Kokkos::make_pair(0, num_local_edges + num_ghost_edges));
    auto xyz =
        Kokkos::subview(this->xyz, Kokkos::make_pair(0, 3 * (num_local_edges + num_ghost_edges)));
    auto r = Kokkos::subview(this->r, Kokkos::make_pair(0, num_local_edges + num_ghost_edges));
    Kokkos::parallel_for(
        "populate edge-based views", num_local_nodes + num_ghost_nodes,
    ...
    auto neigh_ii_indices =
        Kokkos::subview(this->neigh_ii_indices, Kokkos::make_pair(0, num_local_edges));
    Kokkos::parallel_for(
        "populate neigh_ii_indices", num_local_edges, KOKKOS_LAMBDA(const int ij) {
            const int j = neigh_indices(ij);
    ...
    if (mace->has_zbl)
        mace->zbl.compute_ZBL(num_local_nodes, node_types, num_neigh, neigh_types,
    ...
    mace->compute_A0_scaled(num_local_nodes + num_ghost_nodes, node_types, num_neigh, neigh_types,
                            r);
    ...
    mace->reverse_A0_scaled(num_local_nodes + num_ghost_nodes, node_types, num_neigh, neigh_types,
                            xyz, r);
    ...
    if (eflag_global) {
        ...
        double energy;
        Kokkos::parallel_reduce(
            "energy reduction", num_local_nodes,
            KOKKOS_LAMBDA(const int i, double& energy) { energy += node_energies(i); }, energy);
        eng_vdwl += energy;
    }
    ...
    if (eflag_atom) {
        ...
        auto node_energies = mace->node_energies;
        Kokkos::parallel_for(
            "extract atomic energies", num_local_nodes,
            KOKKOS_LAMBDA(const int ii) { d_eatom(ii) += node_energies(ii); });
        k_eatom.modify<DeviceType>();
    }
    ...
    auto mace_node_forces = mace->node_forces;
    Kokkos::parallel_for(
        "force reduction", Kokkos::TeamPolicy<>(num_local_nodes + num_ghost_nodes, Kokkos::AUTO),
        KOKKOS_LAMBDA(Kokkos::TeamPolicy<>::member_type team_member) {
            const int ii = team_member.league_rank();
            const int i = node_indices(ii);
            double f_x, f_y, f_z;
