...
def extract_mace_data(model, species, head=None, num_spline_points=256):
    ...
    model = torch.load(model, map_location=device, weights_only=False).to(
        ...
        for corr in range(1, corr_in_max + 1):
            ...
            try:
                U_matrix = U_matrix_real(
                    irreps_in, [irrep_out], corr, use_cueq_cg=False
                )[1]
            except TypeError:
        ...
        for m1 in range(-l1, l1 + 1):
            ...
            for l2 in range(L_max + 1):
                for m2 in range(-l2, l2 + 1):
                    R = torch.ones(
                        [1, len(tp.instructions) * num_channels],
                        dtype=torch.double,
                        device=device,
                    )
                    ...
                    Y[0, lm1] = 1.0
                    H = torch.zeros(
                        [1, num_lm2 * num_channels], dtype=torch.double, device=device
                    )
                    H[
