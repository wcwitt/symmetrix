...
void bind_multivariate_polynomial_kokkos(py::module_& m) {
    py::class_<MultivariatePolynomialKokkos>(m, "MultivariatePolynomialKokkos")
        ...
        .def("evaluate",
             [](MultivariatePolynomialKokkos& self, py::array_t<const double> x) {
        ...
        .def("evaluate_gradient",
             [](MultivariatePolynomialKokkos& self, py::array_t<const double> x,
                py::array_t<double> g) {
                 ...
                 for (int i = 0; i < g.size(); ++i) g.mutable_data()[i] = g_vector[i];
                 return f;
             })
        .def("evaluate_gradient_simple", [](MultivariatePolynomialKokkos& self,
                                            py::array_t<const double> x, py::array_t<double> g) {
            Kokkos::View<double*> g_view("g", g.size());
            set_kokkos_view(g_view, g);
