...
void bind_mace(py::module_& m) {
    py::class_<MACE>(m, "MACE")
        ...
        .def("compute_node_energies_forces",
             [](MACE& self, const int num_nodes, py::array_t<int> node_types,
                ...
                py::array_t<int> neigh_types, py::array_t<double> xyz, py::array_t<double> r) {
                 self.compute_node_energies_forces(
                     num_nodes, std::span<const int>(node_types.data(), node_types.size()),
        ...
        .def("compute_Phi1",
             [](MACE& self, const int num_nodes, py::array_t<int> num_neigh,
                py::array_t<int> neigh_indices) {
                 self.compute_Phi1(
                     num_nodes, std::span<const int>(num_neigh.data(), num_neigh.size()),
                     std::span<const int>(neigh_indices.data(), neigh_indices.size()));
             })
        .def("reverse_Phi1",
             [](MACE& self, const int num_nodes, py::array_t<int> num_neigh,
                ...
                bool zero_dxyz, bool zero_H1_adj) {
                 self.reverse_Phi1(
                     ...
                     std::span<const int>(neigh_indices.data(), neigh_indices.size()),
                     std::span<const double>(xyz.data(), xyz.size()),
                     std::span<const double>(r.data(), r.size()), zero_dxyz, zero_H1_adj);
             })
        ...
        .def("reverse_H2",
             [](MACE& self, const int num_nodes, py::array_t<int> node_types, bool zero_H1_adj) {
                 self.reverse_H2(num_nodes,
                                 std::span<const int>(node_types.data(), node_types.size()),
                                 zero_H1_adj);
             })
        .def("compute_readouts", [](MACE& self, const int num_nodes, py::array_t<int> node_types) {
